...
def test_examples(
    ...
    for instance_path in [path
                         for path
                         in common_paths['scopes_pkg_data_dir'].iterdir()
    ...
    pkg_dump_dir = dump_dir / 'scopes_pkg'

    pkg_dump_dir.mkdir(parents=True)
    ...
    for name, sln_text in instances.items():
        ...
        (pkg_dump_dir / f"{name}.json").write_text(
            instance_jsons[name]
        )
    ...
    for name, json_text in instance_jsons.items():
        ...
        assert jsonschema.validate(instance, scopes_pkg_json_schema) is None
